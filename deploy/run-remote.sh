...
if [[ "$environment" == staging ]]; then
  ...
  ssh "${ssh_options[@]}" "$DEPLOY_USER@$DEPLOY_HOST" "set -Eeuo pipefail; source $(printf %q "$remote_directory/$(basename "$image_file")"); export BACKEND_IMAGE FRONT_IMAGE BACK_IMAGE; DEPLOY_ROOT=/srv/expressa bash $(printf %q "$remote_directory/deploy.sh") --environment $(printf %q "$environment") deploy all"
fi
