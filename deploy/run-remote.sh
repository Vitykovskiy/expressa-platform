...
if [[ "$environment" == staging ]]; then
  [[ "${BOOTSTRAP_ADMIN_PHONE:-}" && "${AUTH_ACCESS_TOKEN_SECRET:-}" && "${AUTH_OTP_PEPPER:-}" ]] || exit 64
  printf '%s\0%s\0%s\0' "$BOOTSTRAP_ADMIN_PHONE" "$AUTH_ACCESS_TOKEN_SECRET" "$AUTH_OTP_PEPPER" |
    ssh "${ssh_options[@]}" "$DEPLOY_USER@$DEPLOY_HOST" "set -Eeuo pipefail; IFS= read -r -d '' BOOTSTRAP_ADMIN_PHONE; IFS= read -r -d '' STAGING_AUTH_ACCESS_TOKEN_SECRET; IFS= read -r -d '' STAGING_AUTH_OTP_PEPPER; source $(printf %q "$remote_directory/$(basename "$image_file")"); export BACKEND_IMAGE FRONT_IMAGE BACK_IMAGE BOOTSTRAP_ADMIN_PHONE STAGING_AUTH_ACCESS_TOKEN_SECRET STAGING_AUTH_OTP_PEPPER; DEPLOY_ROOT=/srv/expressa bash $(printf %q "$remote_directory/deploy.sh") --environment $(printf %q "$environment") deploy all"
else
