...
export async function runStagingSmoke(
  ...
  if (!isPhone(staffPhone))
    throw new Error("SMOKE_STAFF_PHONE is required for staging smoke");
  ...
  await expectStatus(
    baseUrl,
    `${apiPrefix}/backoffice/orders/${order.id}/accept`,
    403,
    "customer accept denial",
    {
      method: "POST",
  ...
  await expectStatus(
    baseUrl,
    `${apiPrefix}/backoffice/orders/${order.id}/issue`,
    409,
    "early issue",
    {
      method: "POST",
  ...
  assertStage(
    await getOrder(baseUrl, order.id, staffAccessToken),
    "CREATED",
    "early issue",
  );
...
async function authenticate(baseUrl, phone, role) {
  await expectStatus(
    baseUrl,
    `${apiPrefix}/auth/otp/request`,
    202,
    `${role} authentication request`,
    {
      method: "POST",
...
async function createOrder(baseUrl, accessToken) {
  const menu = await expectJson(
    baseUrl,
    `${apiPrefix}/public/menu`,
    200,
    "public menu",
  );
...
async function expectStage(
  ...
  const order = await expectJson(
    baseUrl,
    `${apiPrefix}/backoffice/orders/${orderId}/${action}`,
    expectedStatus,
    name,
    {
      method: "POST",
...
async function getOrder(baseUrl, orderId, accessToken) {
  return expectJson(
    baseUrl,
    `${apiPrefix}/backoffice/orders/${orderId}`,
    200,
    "order details",
    {
      method: "GET",
...
function createOrderItem(product) {
  ...
  const variant =
    product.type === "DRINK" && Array.isArray(product.variants)
      ? product.variants.find(
          (candidate) =>
            isRecord(candidate) &&
            candidate.isAvailable === true &&
            typeof candidate.id === "string" &&
            isAmount(candidate.price),
        )
      : null;
  ...
  for (const group of product.modifierGroups) {
    ...
    const defaults = group.options.filter(
      (option) =>
        ...
        isAmount(option.priceDelta),
    );
    if (defaults.length < group.minSelect || defaults.length > group.maxSelect)
      return null;
...
function assertLifecycleEvents(order) {
  if (!Array.isArray(order.events))
    throw new Error("order events: response is invalid");
  if (order.events.length !== 4)
    throw new Error("order events: expected exactly four events");
  ...
  for (const [from, to] of [
    ...
    if (
      !order.events.some(
        (event) => isRecord(event) && event.from === from && event.to === to,
      )
    ) {
