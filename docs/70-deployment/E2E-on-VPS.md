...
После каждого теста в каждом Playwright-проекте, включая ошибку, runtime удаляет test-scoped
...
прогона, а не custom `index.html` и не отдельные HTML-отчёты сценариев. Полный
прогон выполняется в порядке `chromium` (desktop), затем
`mobile-chromium` (эмуляция `Pixel 5`); mobile-проход выполняется также при
ошибках desktop-прохода.
Счётчики report должны покрывать 78 сценариев в каждом проекте, всего 156
результатов.
