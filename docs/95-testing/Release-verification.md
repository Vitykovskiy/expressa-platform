...
снимки, customer-only `POST /api/v2/orders`, идемпотентность и реальный Chromium
...
После push в `main` workflow один раз запускает набор на временном VPS-стенде.
Каждый сценарий остаётся изолированным и передаёт в полный прогон только свой
Playwright blob-артефакт; seed задаёт состояние сценария, но не объединяет
отчёты. Корневой URL публикует один стандартный объединённый Playwright HTML
report полного набора: его счётчики должны соответствовать ожидаемым 78
сценариям. Отсутствующий из-за инфраструктурной ошибки blob даёт диагностику,
а не доказательство Q-E2E. Повторный прогон того же commit оператор запускает
через `Re-run jobs` в GitHub Actions. Успешный реальный прогон служит
доказательством Q-E2E; конфигурация без такого evidence не является завершением
эпика. [Порядок VPS-проверки](../70-deployment/E2E-on-VPS.md).
