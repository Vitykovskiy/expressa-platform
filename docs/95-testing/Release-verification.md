...
3. Customer открывает меню, видит заранее выбранный доступный размер напитка и обязательные добавки, при необходимости меняет их, формирует корзину и авторизуется по телефону.
4. Backend проверяет цены и доступность и создаёт идемпотентный заказ.
5. Barista видит новый заказ и проводит его через все стадии.
6. После получения оплаты сотрудник переводит заказ из стадии `Готов` в стадию `Выдан`.
7. Customer видит текущий статус и историю.
8. Customer повторяет выданный заказ; корзина учитывает текущие цены и доступность.
9. Customer получает push-уведомления о стадиях `Принят`, `Готов` и `Выдан`.
10. Barista получает push-уведомление о новом заказе.
11. Administrator управляет категориями, товарами, размерами напитков, добавками и ценами.
12. Barista управляет доступностью и приёмом новых заказов.
13. Аудит содержит сотрудников и время критических действий.
14. Полный smoke-сценарий проходит на чистой базе, в `development` и `staging`.
15. CI каждого репозитория выполняет обязательные проверки.
16. Документация каждого репозитория соответствует выпущенному коду.
17. `staging` разворачивается только по Git-тегам версий и хранит точный набор версий трёх сервисов.
