...
После push в `main` workflow один раз запускает набор на временном VPS-стенде и
публикует по корневому URL единственный актуальный Playwright report либо
диагностическую страницу до запуска Playwright. Повторный прогон того же commit
оператор запускает через `Re-run jobs` в GitHub Actions. Успешный реальный прогон
служит доказательством Q-E2E; конфигурация без такого evidence не является
завершением эпика. [Порядок VPS-проверки](../70-deployment/E2E-on-VPS.md).
