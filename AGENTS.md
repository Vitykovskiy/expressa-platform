...
- Временное правило для всего репозитория: не изменять существующие тесты и не
  писать новые без прямого запроса пользователя. Покрывать тестами непринятый
  функционал не имеет смысла. Запускать существующие тесты для проверки можно.
  Правило действует до его явной отмены пользователем и имеет приоритет над
  требованиями вложенных `AGENTS.md` о добавлении или изменении тестов.
