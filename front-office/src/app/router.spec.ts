...
describe('маршруты front-office', () => {
  it.each([
    ...
    expect(testRouter.currentRoute.value.matched[0]?.components?.default).toBeDefined()
    expect(testRouter.currentRoute.value.matched[0]?.meta.title ?? title).toBe(title)
  })
})
