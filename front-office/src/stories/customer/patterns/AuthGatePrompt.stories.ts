...
import type { AuthGatePromptProps } from "../../../customer/pages/auth/AuthGatePrompt.types";
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Запрос подтверждения перед защищённым действием. Контракт: title, message, note, confirmLabel и confirm; переход после confirm решает host. Показывает длинный текст без потери действия. Accessibility: кнопка имеет имя. Источник: src/customer/pages/auth/AuthGatePrompt.vue.",
      },
    },
  },
  render: (args) => ({
