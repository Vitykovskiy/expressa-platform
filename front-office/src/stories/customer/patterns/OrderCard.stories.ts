...
import OrderCard from "../../../customer/pages/orders/OrderCard.vue";
import { createCustomerDefaults } from "../fixtures/customer.fixtures";
import type { Order } from "../../../customer/shared/model/customer.types";
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Карточка заказа. Контракт: order, statusLabel, expanded и toggle; история управляет раскрытием и обновлением. Показывает статусы и длинные позиции; не вычисляет заказ. Accessibility: toggle сообщает aria-expanded. Источник: src/customer/pages/orders/OrderCard.vue.",
      },
    },
  },
  render: (args) => ({
