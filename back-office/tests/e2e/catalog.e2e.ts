...
test("Q-046: edit-draft product type focus stays inside the 320px viewport", async ({
  ...
  try {
    ...
    await archiveCategory(page, categoryName);
    await context.close();
  }
});
...
async function archiveCategory(page: CatalogPage, name: string): Promise<void> {
  await page.goto("/menu");
  await page
    .getByRole("button", { name: `Редактировать категорию ${name}` })
    .click();
  const dialog = page.locator(".edit-dialog:visible");
  await dialog.getByRole("button", { name: "Архивировать категорию" }).click();
  ...
  const [response] = await Promise.all([
    page.waitForResponse(
      (candidate) =>
        ...
        /\/api\/v2\/backoffice\/catalog\/categories\/.+/.test(candidate.url()),
    ),
    confirmation.getByRole("button", { name: "Архивировать" }).click(),
  ]);
  expect(response.status()).toBe(204);
  await expect(categoryToggle(page, name)).toBeHidden();
}
