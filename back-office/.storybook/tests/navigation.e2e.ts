...
const reference = JSON.parse(
  readFileSync(
    new URL("../scripts/reference-index.json", import.meta.url),
    "utf8",
  ),
...
const storyViewports = new Map([
  ["admin-orders-screen--filters-and-refresh", { height: 900, width: 390 }],
]);
...
for (const storyId of storyIds) {
  test(`reference story завершается: ${storyId}`, async ({ page }) => {
    ...
    const viewport = storyViewports.get(storyId);
