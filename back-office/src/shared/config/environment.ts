type ApplicationEnvironment = 'local' | 'development' | 'staging' | 'production'

type EnvironmentSource = Pick<ImportMetaEnv, 'VITE_APP_ENV' | 'VITE_API_BASE_URL'>
...
function validateApplicationEnvironment(value: string): asserts value is ApplicationEnvironment {
...
export function validateEnvironment(environment: EnvironmentSource): EnvironmentConfig {
  const applicationEnvironment = requireEnvironmentValue(
    environment.VITE_APP_ENV,
    'VITE_APP_ENV',
  )
  const apiBaseUrl = requireEnvironmentValue(environment.VITE_API_BASE_URL, 'VITE_API_BASE_URL')
