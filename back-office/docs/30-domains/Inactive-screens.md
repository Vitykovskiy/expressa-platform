...
`AvailabilityScreen`, `OrdersScreen`, `SettingsScreen`, `UsersScreen`, `AddUserDialog` и `UserActionDialog` существуют в `src/pages/admin`, но не импортируются маршрутизатором. Это orphan runtime-компоненты, доступные для ручного просмотра через Storybook; их stories не создают активных маршрутов, API-вызовов или ролевых прав приложения. Источники: [router.constants.ts](../../src/app/router.constants.ts), [stories](../../.storybook/stories/admin/).
