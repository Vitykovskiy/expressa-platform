...
Пользователь вводит 11 цифр российского номера: страница форматирует их как `+7`, не отправляет неверный номер и выводит локальную ошибку. После успешного `requestOtp` доступен шестизначный код; «изменить номер» очищает OTP, «повторить» очищает оба черновика. Неверная длина кода не вызывает API. Cooldown общий с customer для того же номера; при `429` UI показывает предоставленный сервером остаток. Неверный и устаревший OTP остаются на OTP-шаге с отдельным понятным сообщением, а `401` refresh/protected resource очищает сессию. `loading` блокирует форму, `denied` означает, что номер не provisioned как `barista` или `administrator`, а успешная staff-сессия переводит на `/queue`. Источники: [LoginPage.vue](../../src/pages/LoginPage.vue), [session.store.ts](../../src/app/session.store.ts).
