...
`/login` — активный маршрут. `LoginPage` владеет черновиками телефона и OTP, вызывает session store и отображает `AuthScreen` как дочерний UI; `AuthScreen` не orphan и не Storybook-only. Источники: [LoginPage.vue](../../src/pages/LoginPage.vue), [AuthScreen.vue](../../src/pages/admin/auth/AuthScreen.vue), [маршрут](../../src/app/router.constants.ts).
...
`AuthScreen` показывает phone, otp, loading, denied и success states; поля имеют подписанные labels, ошибка представлена наблюдаемым текстом, а адаптивность и отсутствие горизонтальной прокрутки проверяются storybook navigation tests. Это presentation-компонент: HTTP, навигация и сессия остаются у `LoginPage` и store. Источники: [AuthScreen.vue](../../src/pages/admin/auth/AuthScreen.vue), [Auth stories](../../.storybook/stories/admin/auth/AuthScreen.stories.ts), [navigation tests](../../.storybook/tests/navigation.e2e.ts).
