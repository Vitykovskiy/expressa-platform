...
`AuthScreen` показывает phone, otp, loading, denied и success states; поля имеют подписанные labels, ошибка представлена наблюдаемым текстом. Storybook показывает эти состояния вручную. Это presentation-компонент: HTTP, навигация и сессия остаются у `LoginPage` и store. Источники: [AuthScreen.vue](../../src/pages/admin/auth/AuthScreen.vue), [Auth stories](../../.storybook/stories/admin/auth/AuthScreen.stories.ts).
