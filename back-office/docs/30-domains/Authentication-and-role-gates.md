...
Пользователь вводит 11 цифр российского номера: страница форматирует их как `+7`, не отправляет неверный номер и выводит локальную ошибку. После успешного `requestOtp` доступен шестизначный код; «изменить номер» очищает OTP, «повторить» очищает оба черновика. Неверная длина кода не вызывает API. `loading` блокирует форму, ошибки запроса или проверки кода показывают безопасное понятное действие повторить без технического текста API, `denied` показывает отказ, а успешная staff-сессия переводит на `/queue`. Источники: [LoginPage.vue](../../src/pages/LoginPage.vue), [auth E2E](../../tests/e2e/auth.e2e.ts).
