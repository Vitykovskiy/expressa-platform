...
- Каждое системное UI-значение из конечного набора выражается TypeScript `enum` владельца Page/Component Object, а публичный API принимает этот `enum`. Строковый литерал, общий `string`, string literal union, constants-object и отдельный метод для каждого значения не заменяют `enum` и запрещены. Свободные данные остаются `string`; технические ключи и строки локаторов остаются private-деталью объекта. Полное правило — в [Code Style](docs/80-conventions/Code-style.md).
...
- Перед сдачей запускай применимые команды из `package.json`: `npm run lint`, `npm run format:check`, `npm run typecheck:e2e`. Полные E2E-сценарии запускай только при предоставленных URL.
