...
test("ORDER-01: customer видит снимок созданного заказа", async ({
  ...
  await test.step("Customer видит номер и стадию «Оформлен» сохранённого заказа.", async () => {
  ...
  await test.step("Customer видит сохранённые наименование «Капучино», размер M, добавку «Обычное молоко» и количество 1.", async () => {
    const order = await customerOrder.details.readSnapshot();
