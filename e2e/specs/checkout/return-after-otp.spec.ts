 ...
 * - Корзина показывает количество 1 и итог 320,00 ₽.
...
test("CHECKOUT-02: гость возвращается к оформлению после OTP", async ({
  ...
  await test.step("Корзина показывает количество 1 и итог 320,00 ₽.", async () => {
    expect(
    ...
    expect(await checkout.cart.readTotal(), "Показан итог 320,00 ₽.").toBe(
      "320,00 ₽",
    );
  });
