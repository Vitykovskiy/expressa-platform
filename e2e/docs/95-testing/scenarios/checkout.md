...
- Корзина показывает количество один и итог 320 ₽.
...
- `E2E_PROFILE=mutating`, seed-сценарий `canonical` предоставляет «Капучино» размера M с
  ценой 320 ₽; корзина нового browser context пуста.
...
8. Administrator устанавливает цену размера M 300 ₽.
9. Customer открывает корзину.
10. Customer выбирает оформление заказа.
11. Customer подтверждает новый итог 300 ₽.

## Ожидаемый результат

- На desktop корзина показывает блок `Сводка заказа`, на mobile — `Изменение итога заказа`; оба блока содержат прежний итог 320 ₽ и новый итог 300 ₽.
