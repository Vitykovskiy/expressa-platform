...
export type CustomerOrder = OrderQueueItem & { snapshot: readonly OrderSnapshotItem[] };
export type CustomerOrdersPage = { orders: readonly CustomerOrder[]; nextCursor: CustomerOrdersCursor | null };
...
export interface OrderReadRepository {
  ...
  findDetailsForCustomer(customerId: string, orderId: string): Promise<CustomerOrder | null>;
}
