import type { CustomerOrder, CustomerOrdersCursor, CustomerOrdersPage, ListOrdersQuery, OrderReadRepository } from './order-lifecycle.types';
...
export class GetOrdersUseCase {
  ...
  listForCustomer(customerId: string, cursor: CustomerOrdersCursor | null): Promise<CustomerOrdersPage> {
    return this.repository.listForCustomer(customerId, cursor);
  }

  async detailsForCustomer(customerId: string, orderId: string): Promise<CustomerOrder> {
    const order = await this.repository.findDetailsForCustomer(customerId, orderId);
    if (order === null) throw new OrderNotFoundError();
    return order;
  }
}
