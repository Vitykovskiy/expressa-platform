...
describe('GetOrdersUseCase', () => {
  it('передаёт фильтр очереди в репозиторий', async () => {
    ...
    const repository = createRepository({ list: jest.fn().mockResolvedValue(expected) });
  ...
  it('сигнализирует отсутствие деталей заказа', async () => {
    const repository = createRepository({ findDetails: jest.fn().mockResolvedValue(null) });
    const useCase = new GetOrdersUseCase(repository);
  ...
  it('читает только страницу заказов customer с курсором', async () => {
    ...
    const repository = createRepository({ listForCustomer: jest.fn().mockResolvedValue(page) });
    const useCase = new GetOrdersUseCase(repository);
    const cursor = { createdAt: '2030-01-02 03:04:05.123456+00', id: '6f7ef502-6ee5-4b27-84db-a118d9c710de' };

    await expect(useCase.listForCustomer('customer-id', cursor)).resolves.toBe(page);
    expect(repository.listForCustomer).toHaveBeenCalledWith('customer-id', cursor);
  });

  it('не раскрывает чужой заказ customer', async () => {
    const repository = createRepository({ findDetailsForCustomer: jest.fn().mockResolvedValue(null) });
    const useCase = new GetOrdersUseCase(repository);

    await expect(useCase.detailsForCustomer('customer-id', 'order-id')).rejects.toBeInstanceOf(OrderNotFoundError);
  });
});

function createRepository(overrides: Partial<OrderReadRepository>): OrderReadRepository {
  return {
    list: jest.fn(),
    findDetails: jest.fn(),
