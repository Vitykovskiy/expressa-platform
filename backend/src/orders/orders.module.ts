...
import {
  ...
  type OrderNotificationPort,
} from "./application/order-notification-port.types";
  ...
  providers: [
    {
      ...
      useFactory: (database: DatabaseService) =>
        new PostgresOrderUnitOfWork({ pool: createPoolProxy(database) }),
    },
    {
      ...
      useFactory: (
        ...
        push: OrderNotificationPort,
        metrics: OrderMetricsPort,
      ) => new CreateOrderUseCase(unitOfWork, push, metrics),
    },
    { provide: orderNotificationPort, useExisting: SendOrderPushUseCase },
    {
      ...
      useFactory: (): OrderMetricsPort => ({
        ...
        recordOrderTransition: (stage) =>
          ObservabilityMetrics.recordOrderTransition(stage),
      }),
    },
    {
      ...
      inject: [DatabaseService],
      useFactory: (database: DatabaseService) =>
        new PostgresOrderLifecycleRepository({
    ...
    },
    {
      ...
      useFactory: (repository: OrderReadRepository) =>
    ...
    },
    {
      ...
      useFactory: (
        unitOfWork: OrderTransitionUnitOfWork,
        push: OrderNotificationPort,
        metrics: OrderMetricsPort,
      ) => new TransitionOrderUseCase(unitOfWork, push, metrics),
    },
  ],
})
