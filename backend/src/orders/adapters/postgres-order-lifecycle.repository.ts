...
} from "../application/order-lifecycle.types";
import {
  OrderNotFoundError,
...
import type {
  DatabaseRow,
  PostgresOrderLifecycleDependencies,
  TransactionClient,
} from "./postgres-order-lifecycle.types";

export class PostgresOrderLifecycleRepository
  implements OrderReadRepository, OrderTransitionUnitOfWork
{
  constructor(
    private readonly dependencies: PostgresOrderLifecycleDependencies,
  ) {}
  ...
  async list(query: ListOrdersQuery): Promise<readonly OrderQueueItem[]> {
    const result = await this.dependencies.pool.query<DatabaseRow>(
      ...
      [query.stage ?? null, query.number ?? null],
    );
  ...
  async listForCustomer(
    ...
    return {
      orders,
      nextCursor:
        result.rows.length > rows.length && last !== undefined
          ? {
              createdAt: readString(last, "cursor_created_at"),
              id: readString(last, "id"),
            }
          : null,
    };
  }

  async findDetailsForCustomer(
    customerId: string,
    orderId: string,
    ...
    if (result.rows.length !== 1)
      throw new Error("Invalid PostgreSQL customer order details row count");
  ...
  async transition(command: TransitionOrderCommand): Promise<OrderDetails> {
    ...
    try {
      ...
      if (updated.rows.length === 0) {
        const exists = await client.query<DatabaseRow>(
          "SELECT id FROM orders WHERE id = $1",
          [command.orderId],
        );
        throw exists.rows.length === 0
          ? new OrderNotFoundError()
          : new OrderStageConflictError();
      }
      await client.query(
      ...
      if (details === null)
        throw new Error("PostgreSQL did not persist transitioned order");
...
async function readDetails(
  ...
  const order = await client.query<DatabaseRow>(
    `SELECT orders.id, orders.number, orders.created_at, orders.total, orders.stage,
     ...
     FROM orders JOIN users ON users.id = orders.customer_id WHERE orders.id = $1`,
    [orderId],
  );
  const row = order.rows[0];
  if (row === undefined) return null;
  if (order.rows.length !== 1)
    throw new Error("Invalid PostgreSQL order details row count");
  const [snapshot, events] = await Promise.all([
    readSnapshot(client, orderId),
    readEvents(client, orderId),
  ]);
  return {
    ...toQueueItem(row),
    customer: {
      id: readString(row, "customer_id"),
      phoneE164: readString(row, "customer_phone_e164"),
    },
    snapshot,
    events,
  };
...
async function readCustomerOrders(
  client: Queryable,
...
async function readSnapshot(
  ...
  const [items, modifiers] = await Promise.all([
    client.query<DatabaseRow>(
      `SELECT id, product_id, variant_id, product_name, size, quantity, unit_total, line_total FROM order_items WHERE order_id = $1 ORDER BY sort_order`,
      [orderId],
    ),
    client.query<DatabaseRow>(
      `SELECT order_item_id, modifier_option_id, modifier_name, price_delta FROM order_item_modifiers WHERE order_item_id IN (SELECT id FROM order_items WHERE order_id = $1) ORDER BY order_item_id, sort_order`,
      [orderId],
    ),
  ]);
...
function snapshotsByOrder(
  items: readonly DatabaseRow[],
  modifiers: readonly DatabaseRow[],
): ReadonlyMap<string, readonly OrderSnapshotItem[]> {
  ...
  return new Map(
    [...byOrder].map(([orderId, orderItems]) => [
      orderId,
      snapshotItems(orderItems, modifiers),
    ]),
  );
}
...
function snapshotItems(
  ...
  return items.map((row) => ({
    ...
    modifiers: byItem.get(readString(row, "id")) ?? [],
  }));
}
...
async function readEvents(
  client: Queryable,
  orderId: string,
): Promise<readonly OrderEvent[]> {
  const result = await client.query<DatabaseRow>(
    `SELECT e.actor_id, u.phone_e164 AS actor_label, e.occurred_at, e.from_stage, e.to_stage
    FROM order_events e
  ...
  return result.rows.map((row) => ({
    ...
    actorLabel: readString(row, "actor_label"),
    occurredAt: readDate(row, "occurred_at"),
    from: readStage(row, "from_stage"),
    to: readStage(row, "to_stage"),
  }));
}

function toQueueItem(row: DatabaseRow): OrderQueueItem {
  return {
    id: readString(row, "id"),
    number: readString(row, "number"),
    createdAt: readDate(row, "created_at"),
    total: readNonNegativeInteger(row, "total"),
    stage: readStage(row, "stage"),
  };
}
...
function readPositiveInteger(row: DatabaseRow, key: string): number {
  ...
  if (value < 1) throw new Error("Invalid PostgreSQL row field: " + key);
  return value;
}
