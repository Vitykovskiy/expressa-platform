...
describe("PostgresOrderLifecycleRepository", () => {
  it("меняет стадию и записывает единственное событие в одной транзакции", async () => {
    const query = jest.fn((sql: string) => {
      ...
      if (sql.includes("FROM orders JOIN users"))
        return Promise.resolve({
          rows: [
            {
              id: orderId,
              number: "20300102-001",
              created_at: new Date("2030-01-02T03:04:05.000Z"),
              total: 450,
              stage: "ACCEPTED",
              customer_id: "customer-id",
              customer_phone_e164: "+79991234567",
            },
          ],
        });
      if (sql.includes("FROM order_items"))
        return Promise.resolve({ rows: [] });
      if (sql.includes("FROM order_item_modifiers"))
        return Promise.resolve({ rows: [] });
      if (sql.includes("FROM order_events"))
        return Promise.resolve({
          rows: [
            {
              actor_id: "actor-id",
              actor_label: "+79991234567",
              occurred_at: new Date("2030-01-02T03:04:05.000Z"),
              from_stage: "CREATED",
              to_stage: "ACCEPTED",
            },
          ],
    ...
    const repository = new PostgresOrderLifecycleRepository({
      pool: { connect: jest.fn().mockResolvedValue(client) } as unknown as Pool,
    });
    ...
    expect(query).toHaveBeenCalledWith(
      expect.stringContaining("UPDATE orders SET stage"),
      [orderId, "CREATED", "ACCEPTED"],
    );
    expect(query).toHaveBeenCalledWith(
      expect.stringContaining("INSERT INTO order_events"),
      [orderId, "actor-id", expect.any(Date), "CREATED", "ACCEPTED"],
    );
    expect(query).toHaveBeenCalledWith(
      expect.stringContaining("INNER JOIN users u ON u.id = e.actor_id"),
      [orderId],
    );
  ...
  it("откатывает переход, если стадия не соответствует ожидаемой", async () => {
    const query = jest.fn((sql: string) =>
      sql.includes("UPDATE orders")
        ...
        : sql.includes("SELECT id FROM orders")
          ? Promise.resolve({ rows: [{ id: orderId }] })
          : Promise.resolve({ rows: [] }),
    );
    const client = { query, release: jest.fn() } as unknown as PoolClient;
    const repository = new PostgresOrderLifecycleRepository({
  ...
  it("откатывает смену стадии, если не записалось событие", async () => {
    const query = jest.fn((sql: string) => {
      ...
      if (sql.includes("INSERT INTO order_events"))
        return Promise.reject(new Error("event failure"));
      return Promise.resolve({ rows: [] });
    });
    const client = { query, release: jest.fn() } as unknown as PoolClient;
    const repository = new PostgresOrderLifecycleRepository({
          ...
          rows: [
            {
              ...
              cursor_created_at: "2030-01-02 03:04:05+00",
              total: 450,
              stage: "ISSUED",
            },
          ],
          ...
          rows: [
            {
              ...
              modifier_option_id: "88888888-8888-4888-8888-888888888888",
            ...
            snapshot: [
              {
                ...
                modifiers: [
                  {
                    modifierOptionId: "88888888-8888-4888-8888-888888888888",
  ...
  it("читает 20 customer снимков тремя bounded запросами", async () => {
      ...
      if (sql.includes("FROM orders") && sql.includes("customer_id = $1")) {
        return Promise.resolve({
          rows: ids.map((id, index) => ({
            id,
            number: `20310205-${String(index + 1).padStart(3, "0")}`,
            created_at: new Date(
              `2031-02-05T13:00:${String(index).padStart(2, "0")}.000Z`,
            ),
            cursor_created_at: `2031-02-05 13:00:${String(index).padStart(2, "0")}+00`,
            total: 100,
            stage: "CREATED",
          })),
        });
    ...
    expect(
      page.orders.every(
        (order) => order.snapshot[0]?.productName === "Американо",
      ),
    ).toBe(true);
    expect(page.nextCursor).toBeNull();
    expect(query).toHaveBeenCalledTimes(3);
    expect(query).toHaveBeenCalledWith(
      expect.stringContaining("WHERE order_id = ANY($1::uuid[])"),
      [ids],
    );
  });
  ...
  it("сохраняет микросекунды cursor при одинаковой миллисекунде", async () => {
          ...
          rows:
            orderQueryCount === 1
              ? ids.map((id, index) => ({
                  id,
                  number: `20310205-${String(index + 1).padStart(3, "0")}`,
                  created_at: new Date("2031-02-05T13:00:00.123Z"),
                  cursor_created_at:
                    index === 19
                      ? cursorCreatedAt
                      : "2031-02-05 13:00:00.123457+00",
                  total: 100,
                  stage: "CREATED",
                }))
              : [
                  {
                    ...
                    number: "20310205-021",
                    created_at: new Date("2031-02-05T13:00:00.123Z"),
                    cursor_created_at: "2031-02-05 13:00:00.123455+00",
                    total: 100,
                    stage: "CREATED",
                  },
                ],
    ...
    await expect(
      repository.listForCustomer(customerId, firstPage.nextCursor),
    ).resolves.toMatchObject({ orders: [{ id: ids[20] }] });
    ...
    expect(query).toHaveBeenCalledWith(
      ...
      [customerId, cursorCreatedAt, ids[19]],
    );
  });
  ...
  it("не читает customer detail, если customer не владеет заказом", async () => {
    ...
    await expect(
      repository.findDetailsForCustomer("customer-id", orderId),
    ).resolves.toBeNull();
    expect(query).toHaveBeenCalledWith(
