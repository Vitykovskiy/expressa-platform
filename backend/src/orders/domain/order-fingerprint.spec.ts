...
describe("createOrderFingerprint", () => {
  it("канонизирует порядок позиций и добавок", () => {
    ...
    expect(createOrderFingerprint(first)).toBe(
      createOrderFingerprint(sameRequestDifferentOrder),
    );
    expect(createOrderFingerprint(first)).not.toBe(
      createOrderFingerprint({ ...first, total: 101 }),
    );
  });
  ...
  it("использует точный порядок кодовых единиц Unicode", () => {
      ...
      items: [
        {
          productId: composed,
          variantId: null,
          modifierOptionIds: [composed, decomposed],
          quantity: 1,
        },
      ],
    };
      ...
      items: [
        {
          productId: composed,
          variantId: null,
          modifierOptionIds: [decomposed, composed],
          quantity: 1,
        },
      ],
    };
    ...
    expect(createOrderFingerprint(first)).toContain(
      JSON.stringify([decomposed, composed]),
    );
  });
});
