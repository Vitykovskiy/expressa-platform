...
describe("revalidateOrder", () => {
  ...
  it("соблюдает приоритет: приём, валидация, доступность, сумма", () => {
    ...
    expect(() => revalidateOrder({ ...request, items: [] }, closed)).toThrow(
      OrderIntakeClosedError,
    );
    ...
    expect(() =>
      revalidateOrder({ ...request, items: [] }, unavailable),
    ).toThrow(OrderValidationError);

    const changed = createCatalog();
    changed.products[0]!.isAvailable = false;
    expect(() => revalidateOrder({ ...request, total: 1 }, changed)).toThrow(
      MenuItemUnavailableError,
    );

    expect(() =>
      revalidateOrder({ ...request, total: 1 }, createCatalog()),
    ).toThrow(OrderTotalChangedError);
  });

  it("отклоняет неверную конфигурацию и недоступный выбранный вариант", () => {
    expect(() =>
      revalidateOrder(
        { ...request, items: [{ ...request.items[0]!, variantId: null }] },
        createCatalog(),
      ),
    ).toThrow(OrderValidationError);
    expect(() =>
      revalidateOrder(
        {
          ...request,
          items: [{ ...request.items[0]!, modifierOptionIds: [] }],
        },
        createCatalog(),
      ),
    ).toThrow(OrderValidationError);
    expect(() =>
      revalidateOrder(
        { ...request, items: [{ ...request.items[0]!, quantity: 21 }] },
        createCatalog(),
      ),
    ).toThrow(OrderValidationError);
    expect(() =>
    ...
    expect(() => revalidateOrder(request, catalog)).toThrow(
      MenuItemUnavailableError,
    );
  });
  ...
  it("отклоняет неоднозначные данные каталога до выбора первой позиции", () => {
    const catalog = createCatalog();
    const duplicateProduct: OrderCatalog = {
      ...catalog,
      products: [...catalog.products, { ...catalog.products[0]! }],
    };
    expect(() => revalidateOrder(request, duplicateProduct)).toThrow(
      OrderValidationError,
    );
      ...
      products: [
        {
          ...
          variants: [
            ...
            { ...catalog.products[0]!.variants[0]!, id: "other-medium" },
          ],
        },
      ],
    };
    expect(() => revalidateOrder(request, duplicateVariantSize)).toThrow(
      OrderValidationError,
    );
          ...
          modifierGroups: [
            {
              ...
              options: [
                ...
                { ...catalog.products[0]!.modifierGroups[0]!.options[0]! },
              ],
            },
          ],
        },
    ...
    expect(() => revalidateOrder(request, duplicateOption)).toThrow(
      OrderValidationError,
    );
  });
  ...
  it("проверяет товары без вариантов и неизвестные позиции", () => {
    ...
    expect(() =>
      revalidateOrder(
        {
          ...otherRequest,
          items: [{ ...otherRequest.items[0]!, variantId: "x" }],
        },
        catalog,
      ),
    ).toThrow(OrderValidationError);
    expect(() =>
      revalidateOrder(
        {
          ...otherRequest,
          items: [{ ...otherRequest.items[0]!, productId: "missing" }],
        },
        catalog,
      ),
    ).toThrow(OrderValidationError);
  ...
  it("отклоняет ошибки конфигурации групп, опций и выбора модификаторов", () => {
    ...
    expect(() => revalidateOrder(request, invalidGroup)).toThrow(
      OrderValidationError,
    );

    const invalidOption = createCatalog();
    invalidOption.products[0]!.modifierGroups[0]!.options[0]!.name = " ";
    expect(() => revalidateOrder(request, invalidOption)).toThrow(
      OrderValidationError,
    );
    ...
    expect(() =>
      revalidateOrder(
        {
          ...request,
          items: [{ ...request.items[0]!, modifierOptionIds: ["unknown"] }],
        },
        unknownOption,
      ),
    ).toThrow(OrderValidationError);
    ...
    expect(() =>
      revalidateOrder(
        {
          ...request,
          items: [{ ...request.items[0]!, modifierOptionIds: ["oat"] }],
        },
        invalidSelection,
      ),
    ).toThrow(OrderValidationError);
  ...
  it("отклоняет недоступный модификатор и некорректные цены", () => {
    ...
    expect(() => revalidateOrder(request, unavailable)).toThrow(
      MenuItemUnavailableError,
    );

    const negative = createCatalog();
    negative.products[0]!.variants[0]!.price = -1;
    expect(() => revalidateOrder(request, negative)).toThrow(
      OrderValidationError,
    );
