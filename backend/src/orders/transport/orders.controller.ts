...
export class OrdersController {
  ...
  @ApiQuery({
    name: customerOrdersCursorParameter,
    required: false,
    type: "string",
  })
  ...
  async list(
    ...
    const page = await this.getOrders.listForCustomer(
      auth.userId,
      parseCursor(cursor),
    );
    return {
...
function throwOrderError(error: unknown): never {
  ...
  if (error instanceof OrderTotalChangedError) {
    throw new HttpException(
      orderErrorResponses.totalChanged(error.total),
      orderErrorStatus,
    );
  }
  if (error instanceof MenuItemUnavailableError) {
    throw new HttpException(
      orderErrorResponses.unavailable(error.itemId),
      orderErrorStatus,
    );
  }
