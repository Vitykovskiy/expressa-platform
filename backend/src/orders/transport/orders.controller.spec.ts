...
describe('OrdersController', () => {
  ...
  it('требует SessionGuard и роль Customer', () => {
    ...
    expect(Reflect.getMetadata(PATH_METADATA, OrdersController)).toBe(ordersControllerPath);
    ...
    expect(Reflect.getMetadata(GUARDS_METADATA, OrdersController)).toEqual([SessionGuard, RolesGuard]);
    expect(Reflect.getMetadata(rolesMetadataKey, OrdersController)).toBe('Customer');
    expect(Reflect.getMetadata(swaggerParametersMetadataKey, prototype.create)).toContainEqual({
    ...
    expect(Reflect.getMetadata(swaggerResponsesMetadataKey, prototype.create)[400].type).toBe(ApiHttpErrorDto);
  });
  ...
  it('описывает 400 заказа общим HTTP error envelope', async () => {
    ...
    try {
      ...
      expect(document.paths['/orders']?.post?.responses['400']).toMatchObject({
        content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiHttpErrorDto' } } },
      });
      expect(document.components?.schemas?.ApiHttpErrorDto).toMatchObject({
        properties: { details: { type: 'object', nullable: true, additionalProperties: true } },
      });
    } finally {
      await app.close();
    }
  ...
  it('документирует ограничения DTO, совпадающие с transport-проверкой', () => {
    ...
    expect(Reflect.getMetadata(swaggerModelPropertyMetadataKey, CreateOrderItemDto.prototype, 'variantId')).toMatchObject({
      type: 'string', format: 'uuid', nullable: true,
    });
    expect(Reflect.getMetadata(swaggerModelPropertyMetadataKey, CreateOrderItemDto.prototype, 'modifierOptionIds')).toMatchObject({
      type: 'string', format: 'uuid', isArray: true, uniqueItems: true,
    });
    expect(Reflect.getMetadata(swaggerModelPropertyMetadataKey, CreateOrderItemDto.prototype, 'quantity')).toMatchObject({
      type: 'integer', format: 'int32', minimum: 1, maximum: 20,
    });
  });
  ...
  const invalidRequests: readonly [unknown, string, string][] = [
    ...
    [{ ...body, items: [{ ...body.items[0], productId: 'invalid-id' }] }, uuid, 'item id'],
    [{ ...body, items: [{ ...body.items[0], variantId: 'invalid-id' }] }, uuid, 'variant id'],
    [{ ...body, items: [{ ...body.items[0], variantId: undefined }] }, uuid, 'missing variant'],
    [{ ...body, items: [{ ...body.items[0], modifierOptionIds: ['invalid-id'] }] }, uuid, 'option id'],
    [{ ...body, items: [{ ...body.items[0], modifierOptionIds: [uuid, uuid] }] }, uuid, 'duplicate options'],
    [{ ...body, items: [{ ...body.items[0], quantity: 0 }] }, uuid, 'zero quantity'],
    [{ ...body, items: [{ ...body.items[0], quantity: 21 }] }, uuid, 'excess quantity'],
    [{ ...body, items: [{ ...body.items[0], quantity: 1.5 }] }, uuid, 'fractional quantity'],
  ];

  it.each(invalidRequests)('отклоняет невалидный %s', async (invalidBody, idempotencyKey) => {
    const { controller, createOrder } = createController();

    await expect(controller.create(invalidBody as CreateOrderDto, idempotencyKey, auth)).rejects.toMatchObject({
      status: 400,
  ...
  it.each([
    ...
    await expect(failingController.create(body, uuid, auth)).rejects.toMatchObject({
