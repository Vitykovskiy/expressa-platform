...
const details = {
  ...
  customer: { id: "customer-id", phoneE164: "+79990000000" },
  snapshot: [],
  events: [
    {
      actorId: auth.userId,
      actorLabel: auth.phoneE164,
...
describe("BackofficeOrdersController", () => {
  it("отдаёт staff-очередь и детали в каноническом DTO", async () => {
    ...
    const controller = new BackofficeOrdersController(
      ...
      { execute: jest.fn() } as unknown as TransitionOrderUseCase,
      { now: () => new Date() },
    );

    await expect(controller.list("CREATED", "001")).resolves.toEqual([
      {
        id: orderId,
        number: "20300102-001",
    ...
    await expect(controller.details(orderId)).resolves.toMatchObject({
      customer: details.customer,
      snapshot: [],
      events: [{ actorId: auth.userId, actorLabel: auth.phoneE164 }],
    });
  ...
  it.each([
    ...
    [new OrderStageConflictError(), 409, "ORDER_STAGE_CONFLICT"],
  ])("возвращает структуру ошибки lifecycle", async (error, status, code) => {
    const controller = new BackofficeOrdersController(
      { list: jest.fn(), details: jest.fn() } as unknown as GetOrdersUseCase,
      {
        execute: jest.fn().mockRejectedValue(error),
      } as unknown as TransitionOrderUseCase,
      { now: () => new Date() },
    );

    await expect(controller.accept(orderId, auth)).rejects.toMatchObject({
      status,
      response: { code, details: null },
    });
  });
  ...
  it("отклоняет недопустимые параметры до application", async () => {
    ...
    await expect(controller.list("CANCELLED")).rejects.toMatchObject({
      status: 400,
      response: { code: "VALIDATION_ERROR" },
    });
    await expect(controller.details("bad-id")).rejects.toMatchObject({
      status: 400,
      response: { code: "VALIDATION_ERROR" },
    });
  ...
  it("передаёт выдачу заказа как следующий переход", async () => {
    const transitionOrder = {
      execute: jest.fn().mockResolvedValue({ ...details, stage: "ISSUED" }),
    };
    ...
    expect(transitionOrder.execute).toHaveBeenCalledWith({
      orderId,
      action: "issue",
      actorId: auth.userId,
      occurredAt: now,
