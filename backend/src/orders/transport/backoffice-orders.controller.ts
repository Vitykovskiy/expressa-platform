...
function toDetailsDto(order: OrderDetails): BackofficeOrderDetailsDto { return { ...toListDto(order), customer: order.customer, snapshot: order.snapshot.map((item) => ({ ...item, modifiers: [...item.modifiers] })), events: order.events.map((event) => ({ actorId: event.actorId, occurredAt: event.occurredAt.toISOString(), from: event.from, to: event.to })) }; }
