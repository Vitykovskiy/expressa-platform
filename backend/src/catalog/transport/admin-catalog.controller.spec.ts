...
describe("AdminCatalogController", () => {
  ...
  it("регистрирует защищённый GET /backoffice/catalog только для Administrator", () => {
    ...
    expect(Reflect.getMetadata(PATH_METADATA, AdminCatalogController)).toBe(
      adminCatalogControllerPath,
    );
    expect(Reflect.getMetadata(PATH_METADATA, prototype.getCatalog)).toBe("/");
    expect(Reflect.getMetadata(METHOD_METADATA, prototype.getCatalog)).toBe(
      RequestMethod.GET,
    );
    expect(Reflect.getMetadata(GUARDS_METADATA, prototype.getCatalog)).toEqual([
      SessionGuard,
      RolesGuard,
    ]);
    expect(Reflect.getMetadata(rolesMetadataKey, prototype.getCatalog)).toBe(
      "Administrator",
    );
