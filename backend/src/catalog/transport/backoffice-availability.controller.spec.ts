...
describe("BackofficeAvailabilityController", () => {
  ...
  it("передаёт availability и intake staff-команды", async () => {
    ...
    const controller = new BackofficeAvailabilityController(
      get as unknown as GetAdminCatalogUseCase,
      availability as unknown as ManageAvailabilityUseCase,
      intake as unknown as ManageServiceIntakeUseCase,
    );
    ...
    expect(intake.execute).toHaveBeenCalledWith({
      acceptsNewOrders: false,
      actorId: "staff",
      requestId: "request",
    });
  ...
  it("регистрирует staff routes", () => {
    ...
    expect(
      Reflect.getMetadata(PATH_METADATA, BackofficeAvailabilityController),
    ).toBe("backoffice");
    expect(
      Reflect.getMetadata(GUARDS_METADATA, BackofficeAvailabilityController),
    ).toEqual([SessionGuard, RolesGuard]);
    expect(
      Reflect.getMetadata(rolesMetadataKey, BackofficeAvailabilityController),
    ).toBe("Staff");
    expect(Reflect.getMetadata(PATH_METADATA, prototype.getAvailability)).toBe(
      "availability",
    );
    expect(
      Reflect.getMetadata(METHOD_METADATA, prototype.updateAvailability),
