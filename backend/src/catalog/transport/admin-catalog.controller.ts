...
import {
  adminCatalogApiTag,
  adminCatalogControllerPath,
} from "./admin-catalog.controller.constants";
import {
...
function toAdminCatalogDto(catalog: AdminCatalogCandidates): AdminCatalogDto {
  return {
    ...
    categoryModifierGroups: catalog.categoryModifierGroups.map(
      toCategoryModifierGroupDto,
    ),
  };
}

function toCategoryDto(
  category: AdminCatalogCandidates["categories"][number],
): AdminCatalogCategoryDto {
  return {
    id: category.id,
    name: category.name,
...
function toProductDto(
  product: AdminCatalogCandidates["products"][number],
): AdminCatalogProductDto {
  return {
    id: product.id,
...
function toProductVariantDto(
  variant: AdminCatalogCandidates["productVariants"][number],
): AdminCatalogProductVariantDto {
  return {
    id: variant.id,
...
function toModifierGroupDto(
  group: AdminCatalogCandidates["modifierGroups"][number],
): AdminCatalogModifierGroupDto {
  return {
    id: group.id,
    name: group.name,
...
function toModifierOptionDto(
  option: AdminCatalogCandidates["modifierOptions"][number],
): AdminCatalogModifierOptionDto {
  return {
    id: option.id,
...
function toCategoryModifierGroupDto(
  assignment: AdminCatalogCandidates["categoryModifierGroups"][number],
): AdminCatalogCategoryModifierGroupDto {
  return {
