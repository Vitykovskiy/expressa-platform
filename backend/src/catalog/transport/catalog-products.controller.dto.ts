...
export class ProductVariantRequestDto {
  @ApiProperty({ enum: ["S", "M", "L"] }) size!: CatalogProductSize;
  @ApiProperty({ format: "int32", type: "integer", minimum: 0 }) price!: number;
...
export class CreateProductDto {
  ...
  @ApiProperty({ enum: ["DRINK", "OTHER"] }) type!: CatalogProductType;
  @ApiProperty() name!: string;
  @ApiProperty() description!: string;
  @ApiProperty({ format: "int32", type: "integer", minimum: 0, nullable: true })
  price!: number | null;
  @ApiProperty({ format: "int32", type: "integer", minimum: 0 })
  sortOrder!: number;
  @ApiProperty() isActive!: boolean;
  @ApiProperty() isAvailable!: boolean;
  @ApiProperty({ type: () => ProductVariantRequestDto, isArray: true })
