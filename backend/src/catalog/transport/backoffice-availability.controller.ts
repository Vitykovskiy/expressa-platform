...
import { ApiHttpErrorDto, ApiValidationErrorDto } from '../../platform/observability/http-error.dto';
...
import { AvailabilityNotFoundError, ManageAvailabilityUseCase } from '../application/manage-availability.use-case';
...
import { AvailabilityResponseDto, AvailabilityUpdateRequestDto, AvailabilityUpdateResponseDto, ServiceIntakeRequestDto, ServiceIntakeResponseDto } from './backoffice-availability.dto';
import type { AvailabilityRequestContext, AvailabilityRouteType } from './backoffice-availability.dto.types';
...
export class BackofficeAvailabilityController {
  ...
  async getAvailability(): Promise<AvailabilityResponseDto> {
    ...
    if (catalog.intake === undefined) throw new Error('Availability intake is missing');
  ...
  async updateAvailability(
    ...
    assertAvailabilityType(type); assertUuid(id, 'id'); assertBoolean(body?.isAvailable, 'isAvailable');
    try {
      return await this.manageAvailability.execute({ type, id, isAvailable: body.isAvailable, actorId: auth.userId, requestId: requestId(request) });
    } catch (error) {
  ...
  async updateIntake(
    ...
    return this.manageServiceIntake.execute({ acceptsNewOrders: body.acceptsNewOrders, actorId: auth.userId, requestId: requestId(request) });
  }
}

function assertAvailabilityType(value: string): asserts value is AvailabilityEntityType { if (value !== 'product' && value !== 'variant' && value !== 'modifier') throw validationError([{ path: 'type', reason: 'Must be product, variant, or modifier' }]); }
function assertBoolean(value: unknown, path: string): asserts value is boolean { if (typeof value !== 'boolean') throw validationError([{ path, reason: 'Must be a boolean' }]); }
function assertUuid(value: string, path: string): void { if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) throw validationError([{ path, reason: 'Must be a UUID' }]); }
function requestId(request: AvailabilityRequestContext): string { if (typeof request.requestId !== 'string' || request.requestId === '') throw validationError([{ path: 'requestId', reason: 'Must be present' }]); return request.requestId; }
function throwAvailabilityError(error: unknown): never { if (error instanceof AvailabilityNotFoundError) throw new HttpException({ code: 'AVAILABILITY_ITEM_NOT_FOUND', message: 'Availability item not found', details: null }, HttpStatus.NOT_FOUND); throw error; }
