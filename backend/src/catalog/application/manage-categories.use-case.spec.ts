...
function setup() {
  ...
  const run = jest.fn(
    async <Result>(
      command: (value: CategoriesRepository) => Promise<Result>,
      audit: (value: CategoriesRepository, result: Result) => Promise<void>,
    ): Promise<Result> => {
...
describe("ManageCategoriesUseCase", () => {
  ...
  it("отклоняет изменение архивной категории до записи", async () => {
    ...
    ).rejects.toThrow(CategoryAdminError);
  ...
  it("требует полный набор и аудирует каждую категорию после переупорядочивания", async () => {
    ...
    ).rejects.toThrow("CATEGORY_REORDER_INVALID");
