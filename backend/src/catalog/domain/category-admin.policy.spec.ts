...
describe("category admin policy", () => {
  it("отклоняет активную категорию с занятой позицией", () => {
    expect(() =>
      assertAvailableActivePosition([currentCategory], null, {
        name: "Чай",
        description: "Напитки",
        sortOrder: 0,
        isActive: true,
  ...
  it("различает отсутствующую и архивную категории", () => {
    ...
    expect(() =>
      assertCurrentCategory({ ...currentCategory, archivedAt: new Date() }),
    ).toThrow("CATEGORY_ARCHIVED");
  });

  it("требует полный набор текущих категорий для переупорядочивания", () => {
    expect(() =>
      assertFullCategoryReorder(
        [currentCategory, { ...currentCategory, id: "tea" }],
        ["tea"],
      ),
    ).toThrow("CATEGORY_REORDER_INVALID");
    expect(() =>
      assertFullCategoryReorder(
        [currentCategory, { ...currentCategory, id: "tea" }],
        ["tea", "coffee"],
  ...
  it("отклоняет пустое имя и отрицательную позицию", () => {
    ...
    expect(() =>
      assertCategoryDetails({
        name: "Чай",
        description: "",
        sortOrder: maximumCategorySortOrder + 1,
        isActive: true,
      }),
    ).toThrow("CATEGORY_INVALID");
  });
