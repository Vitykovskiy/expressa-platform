...
export class PostgresCategoryModifiersRepository implements CategoryModifiersUnitOfWork {
  ...
  async run<Result>(
    ...
    return this.runner.run(
      (client) => command(new PostgresCategoryModifiersTransactionRepository(client)),
      (client, result) => audit(new PostgresCategoryModifiersTransactionRepository(client), result),
    );
  }
}
...
class PostgresCategoryModifiersTransactionRepository implements CategoryModifiersRepository {
  ...
  async findCurrentModifierGroupIds(groupIds: readonly string[]): Promise<readonly string[]> {
  ...
  async findByCategoryId(categoryId: string): Promise<readonly CategoryModifierGroup[]> {
    const result = await this.client.query<DatabaseRow>(
  ...
  async replace(categoryId: string, groupIds: readonly string[]): Promise<readonly CategoryModifierGroup[]> {
    await this.client.query('DELETE FROM category_modifier_groups WHERE category_id = $1', [categoryId]);
    if (groupIds.length === 0) return [];
    const result = await this.client.query<DatabaseRow>(
    ...
    return result.rows.map(parseAssignment).toSorted((left, right) => left.sortOrder - right.sortOrder);
  }
  ...
  async writeAudit(event: CategoryModifierGroupsAuditEvent): Promise<void> {
    await this.client.query(
      ...
      [event.actorId, event.categoryId, JSON.stringify(event.before), JSON.stringify(event.after), event.requestId],
    );
  }
}
