...
function setup() {
  ...
  return {
    client,
    query,
    repository: new PostgresCategoriesRepository(
...
describe("PostgresCategoriesRepository", () => {
  it("переупорядочивает полный набор временными bounded позициями в одной транзакции", async () => {
    ...
    query
      ...
      .mockResolvedValueOnce({
        rows: [row, { ...row, id: "tea", name: "Чай", sort_order: 1 }],
      });
    const categories = [
      {
        id: "category-id",
    ...
    await expect(
      repository.run(
        (transaction) =>
          transaction.reorder(categories, ["category-id", "tea"]),
        async () => undefined,
      ),
    ).resolves.toEqual([
      {
  ...
  it("создаёт, обновляет и архивирует категории с возвращаемым состоянием", async () => {
    ...
    query.mockImplementation((text: string) => {
      ...
      if (text.includes("archived_at = CURRENT_TIMESTAMP"))
        return Promise.resolve({
          rows: [{ ...row, archived_at: new Date("2026-08-04T00:00:00.000Z") }],
        });
      if (text.includes("SET name ="))
        return Promise.resolve({ rows: [{ ...row, name: "Чай" }] });
    ...
    await expect(
      repository.run(
        (transaction) =>
          transaction.update("category-id", {
            name: "Чай",
            description: "Напитки",
            sortOrder: 0,
            isActive: true,
    ...
    ).resolves.toMatchObject({ name: "Чай" });
    await expect(
      repository.run(
        (transaction) => transaction.archive("category-id"),
        async () => undefined,
      ),
