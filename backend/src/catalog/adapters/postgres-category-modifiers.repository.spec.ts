...
function setup() {
  ...
  return {
    query,
    repository: new PostgresCategoryModifiersRepository(
...
describe("PostgresCategoryModifiersRepository", () => {
  it("заменяет связи в заданном порядке и пишет аудит в той же транзакции", async () => {
    ...
    query.mockImplementation((text: string) => {
      ...
      if (text.includes("FROM modifier_groups"))
        return Promise.resolve({ rows: groupIds.map((id) => ({ id })) });
      if (text.includes("SELECT category_id, group_id, sort_order"))
        return Promise.resolve({
          rows: [
            { category_id: categoryId, group_id: secondGroupId, sort_order: 0 },
          ],
        });
      if (text.includes("INSERT INTO category_modifier_groups"))
        return Promise.resolve({
          rows: after.map((value) => ({
    ...
    await repository.run(
      async (transaction) => {
        ...
        expect(await transaction.findCurrentModifierGroupIds(groupIds)).toEqual(
          groupIds,
        );
    ...
    expect(
      query.mock.calls.some(
        ([text]) =>
          typeof text === "string" &&
          text.includes("DELETE FROM category_modifier_groups"),
      ),
    ).toBe(true);
    const insert = query.mock.calls.find(
      ([text]) =>
        typeof text === "string" &&
        text.includes("INSERT INTO category_modifier_groups"),
    );
    expect(insert?.[1]).toEqual([categoryId, groupIds]);
    const audit = query.mock.calls.find(
      ([text]) =>
        typeof text === "string" && text.includes("INSERT INTO audit_events"),
    );
  ...
  it("не создаёт строк для пустого упорядоченного набора", async () => {
    ...
    await expect(
      repository.run(
        (transaction) => transaction.replace(categoryId, []),
        async () => undefined,
      ),
    ).resolves.toEqual([]);
