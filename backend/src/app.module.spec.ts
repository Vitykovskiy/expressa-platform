...
describe("AppModule", () => {
  ...
  it("создаёт корневой модуль приложения", async () => {
    const { AuthModule }: typeof import("./auth/auth.module") =
      loadModule("./auth/auth.module");
    const { AppModule }: typeof import("./app.module") =
    ...
    expect(
      module.select(AuthModule).get(otpCodeGeneratorPort, { strict: true }),
    ).toBeInstanceOf(DevelopmentOtpAdapter);
    expect(
      module.select(AuthModule).get(smsSenderPort, { strict: true }),
    ).toBeInstanceOf(DevelopmentOtpAdapter);
    expect(
      module.select(CatalogModule).get(GetPublicMenuUseCase, { strict: true }),
    ).toBeInstanceOf(GetPublicMenuUseCase);
    expect(
      module.select(OrdersModule).get(CreateOrderUseCase, { strict: true }),
    ).toBeInstanceOf(CreateOrderUseCase);
  });
});
