...
export const pushSubscriptionRepositoryPort = Symbol(
...
@Module({
  ...
  providers: [
    {
      provide: PostgresPushSubscriptionRepository,
      inject: [DatabaseService],
      useFactory: (database: DatabaseService) =>
        new PostgresPushSubscriptionRepository({
          pool: createPoolProxy(database),
        }),
    },
    {
      provide: pushSubscriptionRepositoryPort,
      inject: [PostgresPushSubscriptionRepository],
      useFactory: (repository: PostgresPushSubscriptionRepository) =>
        repository,
    },
    {
      ...
      useFactory: (configuration: ConfigService) =>
        new WebPushSender({
          subject: configuration.getOrThrow<string>("VAPID_SUBJECT"),
          publicKey: configuration.getOrThrow<string>("VAPID_PUBLIC_KEY"),
          privateKey: configuration.getOrThrow<string>("VAPID_PRIVATE_KEY"),
        }),
    },
    {
      provide: ManagePushSubscriptionUseCase,
      inject: [pushSubscriptionRepositoryPort],
      useFactory: (repository: PushSubscriptionRepository) =>
        new ManagePushSubscriptionUseCase(repository),
    },
    {
      provide: SendOrderPushUseCase,
      inject: [pushSubscriptionRepositoryPort, pushSenderPort],
      useFactory: (
        repository: PushSubscriptionRepository,
        sender: PushSender,
      ) => new SendOrderPushUseCase(repository, sender),
    },
  ],
