...
describe('PostgresPushSubscriptionRepository', () => {
  it('не передаёт endpoint другому пользователю при конфликте', async () => {
    ...
    await repository.upsert({ userId: 'other-user', endpoint: 'https://push.example/subscription', p256dh: 'key', auth: 'auth' });

    expect(query).toHaveBeenCalledWith(expect.stringContaining('WHERE push_subscriptions.user_id = EXCLUDED.user_id'), ['other-user', 'https://push.example/subscription', 'key', 'auth']);
