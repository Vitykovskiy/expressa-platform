...
describe("validateEnvironment", () => {
  ...
  it.each([
    ...
    [
      ...
      { ...validEnvironment, AUTH_ACCESS_TOKEN_SECRET: undefined },
    ],
    ["AUTH_OTP_PEPPER", { ...validEnvironment, AUTH_OTP_PEPPER: "  " }],
    [
      "VAPID_SUBJECT",
      { ...validEnvironment, VAPID_SUBJECT: "invalid-subject" },
    ],
    [
      "VAPID_PUBLIC_KEY",
      { ...validEnvironment, VAPID_PUBLIC_KEY: "not valid" },
    ],
    [
      "VAPID_PUBLIC_KEY",
      { ...validEnvironment, VAPID_PUBLIC_KEY: "dG9vLXNob3J0" },
    ],
    [
      "VAPID_PRIVATE_KEY",
      { ...validEnvironment, VAPID_PRIVATE_KEY: undefined },
    ],
    [
      "AUTH_DEVELOPMENT_OTP",
      { ...validEnvironment, AUTH_DEVELOPMENT_OTP: "12345" },
    ],
    [
      "CORS_ORIGINS",
      {
        ...validEnvironment,
        CORS_ORIGINS: "https://customer.expressa.test/path",
      },
    ],
    [
      "BOOTSTRAP_ADMIN_PHONE",
      { ...validEnvironment, BOOTSTRAP_ADMIN_PHONE: "79991234567" },
    ],
  ...
  it("требует staging test OTP только в staging", () => {
    const staging = {
      ...validEnvironment,
      NODE_ENV: "staging",
      AUTH_DEVELOPMENT_OTP: undefined,
    };

    expectValidationError(staging, "AUTH_OTP_MODE");
    expectValidationError(
      { ...staging, AUTH_OTP_MODE: "sms" },
      "AUTH_OTP_MODE",
    );
    expectValidationError(
      { ...staging, AUTH_OTP_MODE: "staging_test" },
  ...
  it("требует development OTP в local и development", () => {
    ...
    expectValidationError(
      { ...validEnvironment, AUTH_DEVELOPMENT_OTP: undefined },
      "AUTH_DEVELOPMENT_OTP",
    );
    expectValidationError(
      { ...development, AUTH_DEVELOPMENT_OTP: undefined },
      "AUTH_DEVELOPMENT_OTP",
    );
  ...
  it.each([
    ...
    "использует Secure cookie во всех средах, кроме exact local: %s",
