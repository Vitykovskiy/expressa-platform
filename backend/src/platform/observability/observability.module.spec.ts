...
describe("MetricsController", () => {
  it.each([
    ...
    "публикует readiness, когда PostgreSQL %s",
    async (_state, query, expectedReadiness) => {
      const metrics = {
      ...
      await expect(controller.getMetrics()).resolves.toBe(
        "expressa_backend_readiness 1",
      );
      expect(query).toHaveBeenCalledWith("SELECT 1");
      expect(metrics.recordBackendReadiness).toHaveBeenCalledWith(
        expectedReadiness,
      );
    },
  );
