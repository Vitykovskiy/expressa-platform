...
describe('StagingOtpAdapter', () => {
  ...
  it('выдаёт фиксированный код только в staging_test режиме', () => {
    const adapter = new StagingOtpAdapter('staging', 'staging_test', '123456', allowlist);
  ...
  it.each([
    ...
    expect(() => new StagingOtpAdapter(environment, mode, '123456', allowlist)).toThrow(
  ...
  it.each([undefined, '', '12345', '1234567', '12 3456'])(
    ...
    (code) => {
      expect(() => new StagingOtpAdapter('staging', 'staging_test', code, allowlist)).toThrow(
        'Invalid environment variable: STAGING_TEST_OTP_CODE',
      );
    },
  );
  ...
  it.each([undefined, '', '+79991234567,', '+79991234567,+79991234567'])(
    ...
    (allowlistValue) => {
      expect(() => new StagingOtpAdapter('staging', 'staging_test', '123456', allowlistValue)).toThrow(
  ...
  it('разрешает отправку только номеру из allowlist', async () => {
    ...
    await expect(adapter.send('+79991234567', '123456')).resolves.toBeUndefined();
