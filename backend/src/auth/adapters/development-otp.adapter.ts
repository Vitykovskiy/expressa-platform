...
import { developmentEnvironments, otpCodePattern } from './development-otp.adapter.constants';
...
export class DevelopmentOtpAdapter implements OtpCodeGenerator, SmsSender {
  ...
  constructor(
    ...
    if (!isDevelopmentEnvironment(environment)) {
      throw new Error('Development OTP adapter is unavailable in this environment.');
    }
...
function isDevelopmentEnvironment(value: string): value is DevelopmentEnvironment {
