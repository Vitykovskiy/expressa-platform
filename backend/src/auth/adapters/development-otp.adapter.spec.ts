...
describe('DevelopmentOtpAdapter', () => {
  it.each(['local', 'development'])('выдаёт фиксированный код в среде %s', (environment) => {
    const adapter = new DevelopmentOtpAdapter(environment, '123456');

    expect(adapter.generate()).toBe('123456');
  });

  it.each(['staging', 'production'])('не создаётся в среде %s', (environment) => {
  ...
  it('не возвращает код при отправке', async () => {
    ...
    await expect(adapter.send('+79123456789', '123456')).resolves.toBeUndefined();
  });
});
