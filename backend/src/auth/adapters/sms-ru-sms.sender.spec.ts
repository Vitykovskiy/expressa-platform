...
describe('SmsRuSmsSender', () => {
  it('отправляет один POST-запрос в SMS.RU с формой провайдера', async () => {
    ...
    await expect(smsSender.send('+79123456789', '123456')).resolves.toBeUndefined();
    ...
    expect(
      (fetchImplementation.mock.calls[0]?.[1]?.body as URLSearchParams).toString(),
    ).toBe(
  ...
  it.each([
    ...
    new Response(
      JSON.stringify({
        sms: { '79123456789': { sms_id: 'message-id', status: 'ERROR', status_code: 207 } },
        status: 'OK',
        status_code: 100,
      }),
      { status: 200 },
  ...
  it('прерывает единственный запрос через пять секунд', async () => {
    ...
    try {
      ...
      const rejection = expect(delivery).rejects.toThrow('SMS delivery failed.');
