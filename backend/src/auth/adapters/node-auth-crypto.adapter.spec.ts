...
describe('NodeAuthCryptoAdapter', () => {
  ...
  it('отклоняет token с неверной signature, expiry или внешней формой', () => {
    ...
    expect(
      crypto.verifyAccessToken(`${header}.${payload}.invalid`, accessTokenInput),
    ).toBeNull();
    expect(
      crypto.verifyAccessToken(token, {
        ...accessTokenInput,
  ...
  it('создаёт HMAC OTP hash и сравнивает его без раскрытия кода', () => {
    ...
    expect(crypto.verifyOtpHash('challenge-id', '+79990000000', '123456', hash)).toBe(true);
    expect(crypto.verifyOtpHash('challenge-id', '+79990000000', '654321', hash)).toBe(false);
    expect(crypto.verifyOtpHash('challenge-id', '+79990000000', '123456', 'invalid')).toBe(false);
  });
