...
import { OtpDeliveryUnavailableError, RequestOtpUseCase } from '../application/request-otp.use-case';
import { SessionCreationUnavailableError, VerifyOtpUseCase } from '../application/verify-otp.use-case';
...
import { authErrorResponses, otpRetryAfterSeconds } from './auth.controller.constants';
import {
  AccessTokenDto,
  RequestOtpDto,
...
import type { AccessTokenResponse, AuthCookieResponse, AuthHeaderResponse } from './auth.controller.types';
...
export class AuthController {
  ...
  async verifyCode(
    ...
    writeRefreshCookie(response, result.refreshToken, this.getCookieMaxAge(result.sessionExpiresAt));
...
function throwSafeAuthError(error: unknown): never {
  if (error instanceof InvalidPhoneError) {
    throw new HttpException(authErrorResponses.validation, HttpStatus.BAD_REQUEST);
  }

  if (error instanceof InvalidOtpCodeError) {
    throw new HttpException(authErrorResponses.invalidOtp, HttpStatus.UNAUTHORIZED);
  }

  if (error instanceof ExpiredOtpCodeError) {
    throw new HttpException(authErrorResponses.expiredOtp, HttpStatus.UNAUTHORIZED);
  }

  if (error instanceof OtpRateLimitedError) {
    throw new HttpException(authErrorResponses.rateLimited, HttpStatus.TOO_MANY_REQUESTS);
  }

  if (error instanceof OtpDeliveryUnavailableError || error instanceof SessionCreationUnavailableError) {
    throw new HttpException(authErrorResponses.serviceUnavailable, HttpStatus.SERVICE_UNAVAILABLE);
  }
...
function createAccessTokenResponse(accessToken: string): AccessTokenResponse {
  return { accessToken, expiresInSeconds: accessTokenLifetimeMs / 1_000, tokenType: 'Bearer' };
}
