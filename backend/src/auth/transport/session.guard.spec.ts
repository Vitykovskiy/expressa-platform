...
function createGuard(
  ...
  const repository = {
    findSessionWithUser: jest.fn().mockResolvedValue(session),
  ...
  return {
    crypto,
    guard: new SessionGuard(repository, crypto, clock, {
...
describe("SessionGuard", () => {
  it.each([
    ...
    async (authorization) => {
      const { guard, crypto, repository } = createGuard(
        claims,
        sessionWithUser,
      );
  ...
  it("отклоняет invalid JWT, revoked session и изменённый DB user", async () => {
    ...
    const revoked = createGuard(claims, {
      ...sessionWithUser,
      session: { ...sessionWithUser.session, revokedAt: now },
    });
    const changedUser = createGuard(claims, {
      ...sessionWithUser,
      user: {
    ...
    await expect(
      revoked.guard.canActivate(
        context({ headers: { authorization: "Bearer token" } }),
      ),
    ).rejects.toThrow(UnauthorizedException);
    await expect(
      changedUser.guard.canActivate(
        context({ headers: { authorization: "Bearer token" } }),
      ),
    ).rejects.toThrow(UnauthorizedException);
  });
