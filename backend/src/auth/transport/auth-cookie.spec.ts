...
describe("auth-cookie", () => {
  ...
  it("извлекает только точный refresh cookie без percent-decoding", () => {
    expect(
      readRefreshCookie(`other=value; ${refreshCookieName}=${refreshToken}`),
    ).toBe(refreshToken);
    expect(
      readRefreshCookie(
        `${refreshCookieName}=${refreshToken.replace(".", "%2E")}`,
      ),
    ).toBeNull();
  });
  ...
  it.each([
    ...
    "пишет и очищает refresh cookie с одинаковыми атрибутами в %s",
    (environment, secure) => {
