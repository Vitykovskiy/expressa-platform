...
function createController() {
  const requestOtp = {
    execute: jest
      .fn()
      .mockResolvedValue({ expiresInSeconds: 300, retryAfterSeconds: 60 }),
  };
  const verifyOtp = {
...
describe("AuthController", () => {
  ...
  it("публикует OTP request с HTTP 202", async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:${address.port}/auth/otp/request`,
        {
  ...
  it("verify и refresh возвращают только access token и ставят strict cookie с floor maxAge", async () => {
    ...
    try {
      ...
      await expect(
        verify.controller.verifyCode(
          { phone: "+79123456789", code: "123456" },
          verifyResponse,
        ),
      ).resolves.toEqual({
        accessToken: "access",
      ...
      expect(verify.verifyOtp.execute).toHaveBeenCalledWith(
        "+79123456789",
        "123456",
      );
      expect(verifyResponse.cookie).toHaveBeenCalledWith(
        refreshCookieName,
        refreshToken,
      ...
      expect(refreshResponse.cookie).toHaveBeenCalledWith(
        refreshCookieName,
        refreshToken,
        expect.objectContaining({ maxAge: 119999 }),
      );
    } finally {
  ...
  it("refresh превращает absent, malformed и denied cookie в canonical unauthorized", async () => {
    ...
    await expect(
      absent.controller.refresh(undefined, { cookie: jest.fn() }),
    ...
    await expect(
      denied.controller.refresh(`${refreshCookieName}=${refreshToken}`, {
        cookie: jest.fn(),
      }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
  ...
  it("отклоняет raw invalid OTP bodies до use case безопасной validation error", async () => {
    ...
    await expect(
      request.controller.requestCode({ phone: 1 } as never, response),
  ...
  it("мапит known OTP errors в safe HTTP contract и ставит Retry-After", async () => {
    ...
    await expect(
      rateLimited.controller.requestCode({ phone: "+79123456789" }, response),
    ).rejects.toMatchObject({
    ...
    await expect(
      invalidCode.controller.verifyCode(
        { phone: "+79123456789", code: "123456" },
        response,
      ),
    ).rejects.toMatchObject({
    ...
    await expect(
      expiredCode.controller.verifyCode(
        { phone: "+79123456789", code: "123456" },
        response,
      ),
    ).rejects.toMatchObject({
  ...
  it("регистрирует paths и нужные guards на endpoint methods", () => {
    ...
    expect(Reflect.getMetadata(PATH_METADATA, prototype.refresh)).toBe(
      "refresh",
    );
    expect(Reflect.getMetadata(GUARDS_METADATA, prototype.refresh)).toContain(
      OriginGuard,
    );
    expect(
      Reflect.getMetadata(GUARDS_METADATA, prototype.logoutSession),
