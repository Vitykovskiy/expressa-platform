...
import type { AuthCookieOptions, AuthCookieResponse } from './auth.controller.types';

export function readRefreshCookie(value: string | undefined): string | null {
  if (value === undefined || value === '' || value.includes(',') || value.includes('\r') || value.includes('\n')) {
    return null;
  }
...
export function writeRefreshCookie(
  ...
  response.cookie(refreshCookieName, refreshToken, createRefreshCookieOptions(maxAge));
}
...
function isRefreshToken(value: string): boolean {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.[A-Za-z0-9_-]+$/i.test(
    value,
