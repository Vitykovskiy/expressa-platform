...
import type { AuthRepository, AuthSession, AuthUser } from './auth-repository.types';
...
const user: AuthUser = { id: 'user-id', phoneE164: '+79123456789', role: 'customer' };
...
function createUseCase(result: Awaited<ReturnType<AuthRepository['rotateSession']>>) {
  const repository = {
...
describe('RefreshSessionUseCase', () => {
  it('атомарно ротирует refresh token и не продлевает expiry сессии', async () => {
    ...
    await expect(useCase.execute(`${sessionId}.current-secret`)).resolves.toEqual({
  ...
  it.each(['', 'invalid', `${sessionId}.`, `not-a-uuid.current-secret`, `${sessionId}.a.b`])(
    'отклоняет refresh token вне контракта %s',
    async (token) => {
  ...
  it('не выдаёт токены после replay: repository отзывает mismatch', async () => {
    ...
    await expect(useCase.execute(`${sessionId}.replayed-secret`)).rejects.toThrow(
      AccessDeniedError,
    );
  ...
  it('не выдаёт токены для отозванной или истёкшей сессии', async () => {
    ...
    await expect(useCase.execute(`${sessionId}.current-secret`)).rejects.toThrow(
      AccessDeniedError,
    );
    expect(crypto.createAccessToken).not.toHaveBeenCalled();
  });
