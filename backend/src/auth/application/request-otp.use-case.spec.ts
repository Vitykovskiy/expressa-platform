...
import type { AuthRepository, StoredOtpChallenge } from './auth-repository.types';
import type { Clock } from './clock.types';
import type { OtpCodeGenerator } from './otp-code-generator.types';
import { OtpDeliveryUnavailableError, RequestOtpUseCase } from './request-otp.use-case';
...
describe('RequestOtpUseCase', () => {
  it('нормализует номер, сохраняет hash и возвращает только интервалы', async () => {
    ...
    repository.reserveOtpChallenge.mockResolvedValue({ status: 'created', challenge: createChallenge() });
    const crypto = createCrypto();
    crypto.createOtpHash.mockImplementation((challengeId) => 'hash-' + challengeId);
    const codeGenerator: jest.Mocked<OtpCodeGenerator> = { generate: jest.fn(() => '123456') };
    const smsSender: jest.Mocked<SmsSender> = { send: jest.fn().mockResolvedValue(undefined) };
    const clock: Clock = { now: () => now };
    const useCase = new RequestOtpUseCase(repository, codeGenerator, crypto, smsSender, clock);
  ...
  it('не отправляет повторный код в течение cooldown', async () => {
    const repository = createRepository();
    repository.reserveOtpChallenge.mockResolvedValue({ status: 'rate_limited' });
    ...
    await expect(useCase.execute('+79991234567')).rejects.toBeInstanceOf(OtpRateLimitedError);
  ...
  it('закрывает challenge и не раскрывает ошибку SMS', async () => {
    ...
    expect(repository.invalidateOtpChallenge).toHaveBeenCalledWith(expect.any(String), now);
  });
  ...
  it('закрывает только challenge отложенно упавшей отправки', async () => {
    ...
    if (rejectFirstDelivery === undefined || firstChallengeId === undefined || secondChallengeId === undefined) {
      throw new Error('OTP requests were not created');
    }

    rejectFirstDelivery(firstFailure);
    await expect(firstRequest).rejects.toEqual(new OtpDeliveryUnavailableError());
    expect(repository.invalidateOtpChallenge).toHaveBeenCalledWith(firstChallengeId, now);
    expect(repository.invalidateOtpChallenge).not.toHaveBeenCalledWith(secondChallengeId, now);
  });

  it('отправляет SMS только для единственного созданного параллельного challenge', async () => {
    const repository = createRepository();
    repository.reserveOtpChallenge
      .mockResolvedValueOnce({ status: 'created', challenge: createChallenge() })
