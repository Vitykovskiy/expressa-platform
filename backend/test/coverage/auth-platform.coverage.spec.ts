...
function throttleRow(
  ...
  return { attempts: 1, window_started_at: now, ...overrides };
}
...
describe("PostgresAuthRepository: OTP и сессии", () => {
  it("создаёт OTP только после advisory lock и возвращает созданную challenge", async () => {
    const { client, repository } = transactionRepository([
      [],
      [],
      [],
      [throttleRow()],
  ...
  it("не перезаписывает OTP в cooldown", async () => {
    ...
    ).resolves.toEqual({ status: "rate_limited", retryAfterSeconds: 60 });
  ...
  it("различает недоступный, неверный и подтверждённый OTP", async () => {
    ...
    const invalid = transactionRepository([
      [],
      [otpRow({ code_hash: "different" })],
      [otpRow({ attempts: 1 })],
      [],
    ]);
    await expect(
  ...
  it("атомарно rotates или отзывает сессию по текущему хешу", async () => {
    const unavailable = transactionRepository([[], [], []]);
    await expect(
      unavailable.repository.rotateSession(
        "session-id",
        "refresh-hash",
        "next-hash",
        now,
    ...
    const mismatch = transactionRepository([
      [],
      [sessionWithUserRow()],
      [sessionRow({ revoked_at: now })],
      [],
    ]);
    await expect(
      mismatch.repository.rotateSession(
        "session-id",
        "wrong-hash",
        "next-hash",
        now,
    ...
    const rotated = transactionRepository([
      [],
      [sessionWithUserRow()],
      [sessionRow({ refresh_token_hash: "next-hash", rotated_at: now })],
      [],
    ]);
    await expect(
  ...
  it("не принимает неполные строки PostgreSQL как auth-состояние", async () => {
    const pool = {
      query: jest
        .fn()
        .mockResolvedValue({ rows: [{ ...userRow(), role: "owner" }] }),
    };
...
describe("UnifiedExceptionFilter: platform error envelope", () => {
  ...
  it("returns the safe generic envelope for an unknown platform exception", () => {
    ...
    expect(JSON.stringify(result.reply.mock.calls[0][1])).not.toContain(
      "database secret",
    );
  });
