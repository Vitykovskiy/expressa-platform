    ...
    items: [
      {
        ...
        variantId: medium.id,
        modifierOptionIds: [regularMilk.id],
        quantity: 1,
      },
    ],
  };
...
describe("PostgreSQL unit of work заказа", () => {
  ...
  it(
    ...
    async () => {
      ...
      const created = await unitOfWork.createOrder({
        customerId,
        idempotencyKey,
        request,
        now: new Date("2031-02-03T23:59:59.000Z"),
      });

      expect(created).toMatchObject({ replayed: false, order: { total: 320 } });
      expect(created.order.number).toMatch(/^20310203-\d{3}$/);
      await pool.query(
        "UPDATE products SET name = $1, is_available = false WHERE id = $2",
        ["Новое имя", request.items[0]!.productId],
      );

      await expect(
        unitOfWork.createOrder({
          customerId,
          idempotencyKey,
          request,
          now: new Date("2031-02-04T00:00:00.000Z"),
        }),
      ).resolves.toEqual({
        replayed: true,
        order: created.order,
  ...
  it(
    "сохраняет порядок нескольких позиций и добавок одинаково для initial и replay",
    async () => {
      await pool.query(
        `UPDATE modifier_groups SET selection_type = 'multiple', max_select = 2 WHERE id = $1`,
        [catalogSeed.modifierGroups[0]!.id],
      );
      const customerId = await createCustomer(pool);
      const request: OrderRequest = {
        ...
        items: [
          {
            productId: catalogSeed.products[0]!.id,
            variantId: catalogSeed.productVariants[1]!.id,
            modifierOptionIds: [
              catalogSeed.modifierOptions[1]!.id,
              catalogSeed.modifierOptions[0]!.id,
            ],
            quantity: 1,
          },
          {
            productId: catalogSeed.products[2]!.id,
            variantId: null,
            modifierOptionIds: [],
            quantity: 1,
          },
      ...
      expect(created.order.items.map((item) => item.productId)).toEqual(
        request.items.map((item) => item.productId),
      );
      expect(
        created.order.items[0]!.modifiers.map((item) => item.modifierOptionId),
      ).toEqual(request.items[0]!.modifierOptionIds);
    },
    externalProcessTimeoutMs,
  );

  it(
    "разделяет одинаковый idempotency key между customer",
    async () => {
      const key = randomUUID();
      const request = createRequest();
      const [firstCustomerId, secondCustomerId] = await Promise.all([
        createCustomer(pool),
        createCustomer(pool),
      ]);
      const first = await unitOfWork.createOrder({
        customerId: firstCustomerId,
        idempotencyKey: key,
        request,
        now: new Date("2032-01-02T00:00:00Z"),
      });
      const second = await unitOfWork.createOrder({
  ...
  it(
    "после exclusive catalog lock читает новое состояние",
    async () => {
      const customerId = await createCustomer(pool);
      ...
      try {
        ...
        await writer.query(
          ...
          [catalogSeed.products[0]!.id],
        );
        const creation = unitOfWork.createOrder({
          customerId,
          idempotencyKey: randomUUID(),
          request: createRequest(),
          now: new Date("2032-01-03T00:00:00Z"),
        });
  ...
  it(
    "выдаёт уникальные номера конкурентно, отклоняет cap и откатывает counter при ошибке снимка",
    async () => {
      ...
      expect(new Set(results.map((result) => result.order.number)).size).toBe(
        3,
      );
      await pool.query(
        `INSERT INTO order_daily_counters (order_day, last_number) VALUES ('2032-01-05', 999) ON CONFLICT (order_day) DO UPDATE SET last_number = 999`,
      );
      await expect(
        unitOfWork.createOrder({
          customerId: await createCustomer(pool),
          idempotencyKey: randomUUID(),
          request,
          now: new Date("2032-01-05T00:00:00Z"),
        }),
      ).rejects.toThrow("daily order number");
      await pool.query(
        `CREATE FUNCTION fail_order_snapshot() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'snapshot failure'; END; $$`,
      );
      await pool.query(
        `CREATE TRIGGER fail_order_snapshot BEFORE INSERT ON order_items FOR EACH ROW EXECUTE FUNCTION fail_order_snapshot()`,
      );
      ...
      await expect(
        unitOfWork.createOrder({
          ...
          request,
          now: new Date("2032-01-06T00:00:00Z"),
        }),
      ).rejects.toThrow("snapshot failure");
      await expect(
        pool.query(
          `SELECT count(*)::int AS count FROM orders WHERE customer_id = $1 AND idempotency_key = $2`,
          [failedCustomerId, failedKey],
        ),
      ).resolves.toMatchObject({ rows: [{ count: 0 }] });
      await expect(
        pool.query(
          `SELECT count(*)::int AS count FROM order_daily_counters WHERE order_day = '2032-01-06'`,
        ),
      ).resolves.toMatchObject({ rows: [{ count: 0 }] });
  ...
  it(
    ...
    async () => {
      ...
      const [first, second] = await Promise.all([
        unitOfWork.createOrder(command),
        unitOfWork.createOrder(command),
      ]);
