  ...
  it(
    ...
    async () => {
      ...
      await expect(repository.list({ stage: 'CREATED' })).resolves.toEqual(
        expect.arrayContaining([expect.objectContaining({ id: orderId, stage: 'CREATED' })]),
      );
      await expect(repository.transition({ orderId, action: 'accept', actorId: randomUUID(), occurredAt: acceptedAt })).rejects.toMatchObject({ code: '23503' });
