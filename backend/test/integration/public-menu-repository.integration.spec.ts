...
describe('PostgreSQL repository публичного меню', () => {
  ...
  it(
    ...
    async () => {
      ...
      const [cappuccino, espresso, croissant, cheesecake] = catalogSeed.products;
      const [cappuccinoSmall, cappuccinoMedium, cappuccinoLarge, espressoSmall] = catalogSeed.productVariants;
  ...
  it(
    ...
    async () => {
      ...
      const espresso = catalogSeed.products.find((product) => product.name === 'Эспрессо');
      const regularMilk = catalogSeed.modifierOptions.find((option) => option.name === 'Обычное молоко');

      if (cappuccino === undefined || espresso === undefined || regularMilk === undefined) {
        throw new Error('Catalog seed fixture is incomplete');
      }

      await pool.query(`UPDATE product_variants SET is_available = false WHERE product_id IN ($1, $2)`, [
        cappuccino.id,
        espresso.id,
      ]);
      ...
      await expect(useCase.execute()).resolves.toEqual({
        acceptsNewOrders: true,
        categories: [
          expect.objectContaining({ name: 'Выпечка' }),
        ],
      });
    },
    externalProcessTimeoutMs,
  ...
  it(
    ...
    async () => {
      const oatMilk = catalogSeed.modifierOptions.find((option) => option.name === 'Овсяное молоко');

      if (oatMilk === undefined) {
        throw new Error('Catalog seed fixture is incomplete');
      }

      await pool.query(`UPDATE modifier_options SET is_default = true WHERE id = $1`, [oatMilk.id]);
  ...
  it(
    ...
    async () => {
      const cappuccino = catalogSeed.products.find((product) => product.name === 'Капучино');
      const cheesecake = catalogSeed.products.find((product) => product.name === 'Чизкейк');

      if (cappuccino === undefined || cheesecake === undefined) {
        throw new Error('Catalog seed fixture is incomplete');
      }

      await pool.query(`UPDATE products SET archived_at = CURRENT_TIMESTAMP WHERE id = $1`, [cappuccino.id]);
