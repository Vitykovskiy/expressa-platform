...
describe('схема каталога', () => {
  ...
  it(
    ...
    async () => {
      ...
      await expect(
        pool.query(`INSERT INTO categories (name, sort_order) VALUES ('   ', $1)`, [sortOrder + 1]),
      ...
      await pool.query(`UPDATE product_variants SET archived_at = CURRENT_TIMESTAMP WHERE product_id = $1`, [
        productId,
      ]);
      await expect(
        pool.query(
