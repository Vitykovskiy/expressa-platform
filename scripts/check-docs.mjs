...
const SCOPES = {
  ...
  "front-office": {
    docs: "front-office/docs",
    index: "front-office/docs/INDEX.md",
  },
  "back-office": {
...
const SOURCE_FILE_PATTERN =
  /[\w./-]+\.(?:tsx|cjs|mjs|json|yaml|yml|vue|sql|css|ts|js|md)(?=$|[^\w])/g;
...
function walk(directory, predicate = () => true) {
  ...
  return readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isDirectory())
      return EXCLUDED_DIRECTORIES.has(entry.name) ? [] : walk(path, predicate);
...
function applicationRoot(scope) {
  return (
    scope.appRoot ?? (scope.name === "root" ? root : resolve(root, scope.name))
  );
}
...
function readmeStructureViolation(text) {
  ...
  const structures = headings.filter(
    (heading) => heading.title === "Структура каталога",
  );
  if (structures.length !== 1)
    return 'README.md must contain exactly one "## Структура каталога" section';
  if (headings[0]?.index !== structures[0].index)
    return '"## Структура каталога" must be the first level-two heading';

  let treeStart = structures[0].index + 1;
  while (!lines[treeStart]?.trim()) treeStart += 1;
  if (lines[treeStart]?.trim() !== "```text")
    return '"## Структура каталога" must be followed by a fenced text tree';

  const treeEnd = lines.findIndex(
    (line, index) => index > treeStart && line.trim() === "```",
  );
  if (
    treeEnd === -1 ||
    !lines.slice(treeStart + 1, treeEnd).some((line) => line.trim())
  ) {
    return '"## Структура каталога" must be followed by a fenced text tree';
  }
  return null;
...
function readmesIn(directory) {
  return walk(
    directory,
    (file) => relative(directory, file).split(sep).at(-1) === "README.md",
  );
}
...
function trackedNestedReadmes() {
  const paths = execFileSync(
    "git",
    ["ls-files", "-z", "--", "README.md", ":(glob)**/README.md"],
    {
      cwd: root,
      encoding: "utf8",
    },
...
function isKnowledgeNote(file, scope) {
  ...
  if (
    ...
    name === "INDEX.md" ||
    name === "COVERAGE.md" ||
    name.startsWith("_MOC-")
  )
    return false;
  return (
    !local.includes("_journal") &&
...
function escapesWorkspace(from, raw) {
  ...
  if (
    ...
    return false;
  return (
    target.startsWith("/") || !withinWorkspace(resolve(dirname(from), target))
  );
}
...
function resolveTarget(from, raw, docsDirectory) {
  ...
  if (!target.includes("/") && !extname(target))
    candidates.push(resolve(docsDirectory, `${target}.md`));
...
function wikilinks(text) {
  return [
    ...withoutInlineCode(withoutFencedCode(text)).matchAll(/\[\[[^\]]+\]\]/g),
  ].map((match) => match[0]);
}
...
function checkLinks(scope, files) {
  ...
  for (const file of files) {
    ...
    for (const link of wikilinks(text))
      fail(scope.name, file, `Obsidian wikilink is not allowed: ${link}`);
    for (const target of links(text)) {
      ...
      if (resolved && !existsSync(resolved))
        fail(scope.name, file, `broken link: ${target}`);
      if (
        resolved &&
        existsSync(resolved) &&
...
function directSubdirectories(directory) {
  ...
  return readdirSync(directory, { withFileTypes: true })
    .filter(
      (entry) => entry.isDirectory() && !EXCLUDED_DIRECTORIES.has(entry.name),
    )
...
function needsIndex(directory) {
  const notes = readdirSync(directory, { withFileTypes: true })
    ...
    .filter(
      (name) =>
        !["README.md", "INDEX.md", "COVERAGE.md"].includes(name) &&
        !name.startsWith("_MOC-"),
    );
  return (
    notes.length >= 2 ||
    directSubdirectories(directory).some(hasDocumentationContent)
  );
}
...
function linksTo(file, expected, docsDirectory) {
  return links(readFileSync(file, "utf8")).some((target) =>
    resolvesTo(file, target, expected, docsDirectory),
  );
}
...
function checkIndexes(scope) {
  ...
  for (const directory of docsDirectories(scope)) {
    ...
    if (needsIndex(directory) && !existsSync(index)) {
      const moc = readdirSync(directory, { withFileTypes: true }).some(
        (entry) => entry.isFile() && entry.name.startsWith("_MOC-"),
      );
      fail(
        scope.name,
        directory,
        moc
          ? "local _MOC-* cannot replace required INDEX.md"
          : "missing INDEX.md",
      );
      continue;
    }
    if (existsSync(index)) {
      for (const child of directSubdirectories(directory)) {
        ...
        if (
          existsSync(childIndex) &&
          !linksTo(index, childIndex, docsDirectory)
        ) {
          fail(
            scope.name,
            index,
            `missing child INDEX.md link: ${relative(root, childIndex)}`,
          );
        }
      }
    }
    ...
    if (
      existsSync(parentIndex) &&
      !linksTo(index, parentIndex, docsDirectory)
    ) {
      fail(
        scope.name,
        index,
        `missing parent INDEX.md link: ${relative(root, parentIndex)}`,
      );
    }
  }
}
...
function checkEntrypoints(scope) {
  ...
  for (const name of ["README.md", "AGENTS.md", "docs/INDEX.md"]) {
    ...
    if (!existsSync(file))
      fail(scope.name, file, `missing required entrypoint: ${name}`);
  }
}
...
function requiredRootNavigationFiles() {
  ...
  for (const [name, definition] of Object.entries(SCOPES)) {
    ...
    required.push(
      ...docsDirectories(scope)
        .map((directory) => resolve(directory, "INDEX.md"))
        .filter(existsSync),
    );
  }
...
function checkReachability(scope, files) {
  ...
  for (const file of files) {
    if (!isKnowledgeNote(file, scope) || isHistoricalNote(file, scope))
      continue;
    if (!reachable.has(file))
      fail(scope.name, file, `unreachable from ${scope.index}`);
  }
}
...
function sourcePaths(text, file, docsDirectory) {
  ...
  if (metadata.sources) {
    for (const source of metadata.sources.matchAll(SOURCE_FILE_PATTERN))
  ...
  return candidates
    ...
    .map((target) => ({
      target,
      resolved: resolveTarget(file, target, docsDirectory),
    }))
    .filter(
      ({ target, resolved }) =>
        CODE_EXTENSIONS.has(extname(target)) ||
        /(?:^|\/)(?:src|test|tests|openapi|contracts|scripts|config)(?:\/|$)/.test(
          target,
        ) ||
        target.startsWith("."),
    )
...
function checkSources(scope, files, sampleSize) {
  const docsDirectory = resolve(root, scope.docs);
  const sources = files
    .filter((file) => !isHistoricalNote(file, scope))
    .flatMap((file) =>
      sourcePaths(readFileSync(file, "utf8"), file, docsDirectory).map(
        (source) => ({ ...source, file }),
      ),
    );
  const unique = [
    ...new Map(
      sources.map((source) => [`${source.file}\0${source.target}`, source]),
    ).values(),
  ].sort((a, b) =>
  ...
  const inspected =
    sampleSize === null
      ...
      : unique
          .filter(
            (_, index) =>
              index % Math.max(1, Math.ceil(unique.length / sampleSize)) === 0,
          )
          .slice(0, sampleSize);
  for (const source of inspected) {
    if (!source.resolved || !existsSync(source.resolved))
      fail(scope.name, source.file, `missing truth source: ${source.target}`);
  }
...
function sourcePathFromCoverage(cell, file) {
  ...
  for (const target of [...targets, ...quoted]) {
    const resolved = resolveTarget(file, target, docsDirectory);
    if (resolved && CODE_EXTENSIONS.has(extname(resolved)))
...
function coverageRows(coverageFile) {
  const lines = readFileSync(coverageFile, "utf8")
    .split("\n")
    .filter((line) => /^\s*\|/.test(line));
  if (lines.length < 3) return [];
  const headers = splitTableRow(lines[0]).map((cell) =>
...
function registry(scope) {
  ...
  const routeValues = routeFiles.flatMap((file) =>
    [
      ...readFileSync(file, "utf8").matchAll(/\bpath\s*:\s*['"]([^'"]+)['"]/g),
    ].map((match) => match[1]),
  );
  const openapi =
    scope.name === "backend"
      ? resolve(root, "backend/openapi/openapi.json")
      : resolve(appRoot, "contracts/openapi.json");
  let openapiPaths = [];
  if (existsSync(openapi)) {
    try {
      openapiPaths = Object.keys(
        JSON.parse(readFileSync(openapi, "utf8")).paths ?? {},
      );
    } catch {
      openapiPaths = [];
    }
  }
  const sourceObjects = files.filter(
    (file) =>
      !/\.(spec|test|types|constants|dependencies)\.(ts|vue)$/.test(file) &&
      (/\.store\.ts$/.test(file) ||
        /service.*\.ts$/i.test(file) ||
        /(?:Page|Screen|Dialog)\.vue$/.test(file)),
  );
...
function checkCoverage(scope) {
  ...
  for (const required of [
    ...
    if (!fields.includes(required))
      fail(scope.name, coverageFile, `COVERAGE.md missing column: ${required}`);
  }
  ...
  for (const row of rows) {
    ...
    if (
      !status ||
      (!status.includes("n/a") &&
        !/(current|placeholder|orphan|implemented|active)/.test(status))
    ) {
      fail(
        scope.name,
        coverageFile,
        `invalid coverage status: ${row.status || "(empty)"}`,
      );
    }
    if (status.includes("n/a") && !/reason|причин/i.test(joined))
      fail(scope.name, coverageFile, "N/A coverage row lacks reason");
    const note = row["authoritative note"] ?? "";
    const noteTarget = links(note)[0];
    const resolvedNote =
      noteTarget &&
      resolveTarget(coverageFile, noteTarget, dirname(coverageFile));
    if (!noteTarget || !resolvedNote || !existsSync(resolvedNote))
      fail(
        scope.name,
        coverageFile,
        `invalid authoritative note: ${note || "(empty)"}`,
      );
    ...
    for (const value of joined.matchAll(
      /\b(?:GET|POST|PUT|PATCH|DELETE)\s+(\/[^\s|,)`]+)/g,
    ))
      coveredOpenapi.add(value[1]);
  }
  const actual = registry(scope);
  for (const file of actual.sourceObjects)
    if (!coveredFiles.has(normalize(file)))
      fail(
        scope.name,
        coverageFile,
        `unregistered runtime object: ${relative(root, file)}`,
      );
  for (const path of actual.routeValues)
    if (!coveredRoutes.has(path))
      fail(scope.name, coverageFile, `unregistered route: ${path}`);
  for (const path of actual.openapiPaths)
    if (!coveredRoutes.has(path) && !coveredOpenapi.has(path))
      fail(scope.name, coverageFile, `unregistered OpenAPI path: ${path}`);
}
...
function parseArguments(argumentsList) {
  ...
  for (let index = 0; index < argumentsList.length; index += 1) {
    ...
    if (argument === "--scope") selected = argumentsList[++index];
      ...
      sampleSize = Number.parseInt(argumentsList[++index], 10);
  ...
  if (selected !== "all" && !SCOPES[selected])
    throw new Error(`unknown scope: ${selected}`);
...
function runSelfTest() {
  ...
  expected(
    escapesWorkspace(resolve(root, "docs/note.md"), "../../outside.md"),
    true,
    "workspace boundary",
  );
  expected(
    splitTableRow("| [[Note|label]] | current |")[0],
    " [[Note|label]] ",
    "wikilink table cell",
  );
  expected(
    routeReferences("/ /cart").join(","),
    "/,/cart",
    "root and non-root routes",
  );
  expected(
    [..."../../back-office/package.json".matchAll(SOURCE_FILE_PATTERN)][0][0],
...
try {
  const { selected, sampleSize, selfTest } = parseArguments(
  ...
  for (const summary of summaries)
    console.log(
      `${summary.name}: ${summary.files} notes, sampled ${summary.sources.inspected}/${summary.sources.total} truth sources`,
    );
  if (failures.length) {
    for (const failure of failures)
      console.error(`${failure.scope}: ${failure.file}: ${failure.message}`);
    console.error(
      `Documentation check failed: ${failures.length} violation(s).`,
    );
    process.exitCode = 1;
